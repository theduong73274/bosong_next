...
import flagUs from '../../assets/image/flagUs.png';
import flagVn from '../../assets/image/flagVn.png';
...
function HeaderNav({ navLink }) {
	...
	return (
		<div className="flex items-center justify-center uppercase header font-pro gap-x-14 ft:gap-x-10 fs:gap-x-8">
			{navLink.length > 0 &&
				navLink.map((item) => (
					<Link
						key={uuidv4()}
						...
						className={({ isActive }) =>
							isActive
								? 'pb-[2px] border-b-[1.4px] border-primary text-primary'
								: 'pb-[2px]'
						}
					>
						{item.title}
